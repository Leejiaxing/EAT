...
class GIN(torch.nn.Module):
    def __init__(self, num_features, num_classes, num_layers, hidden_dim, dropout, dataset_name):
        ...
        if dataset_name == "ZINC":
            self.node_embedding = nn.Embedding(self.num_features, self.hidden_dim)
        elif "mol" in dataset_name:
            self.node_embedding = AtomEncoder(self.hidden_dim)
        else:
            self.node_embedding = nn.Linear(self.num_features, self.hidden_dim)
        ...
        for _ in range(num_layers - 1):
            self.convs.append(tnn.GINConv(mlp(self.hidden_dim, self.hidden_dim*2, self.hidden_dim), train_eps=True))
