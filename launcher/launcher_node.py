...
class Launcher:
    def __init__(self, model, lr, wd, dataset_name, model_name, device):
    ...
    def train_and_test_standard_split(self, dataset, rounds, max_epochs, tag=""):
        ...
        if "ogbn" in dataset.name:
            ...
            valid_idx = split_idx['valid'].to(self.device)
            test_idx = split_idx['test'].to(self.device)
        else:
            train_idx = data.train_mask.to(self.device)
            valid_idx = data.val_mask.to(self.device)
            test_idx = data.test_mask.to(self.device)
    ...
    def train_epoch(self, data, mask):
        ...
        if 'lpe' in data:
            sign_flip = torch.rand(data.lpe.size(1), device=self.device)
            sign_flip[sign_flip >= 0.5] = 1.0
            sign_flip[sign_flip < 0.5] = -1.0
            data.lpe_ = data.lpe * sign_flip.unsqueeze(0)

        if self.model_name == 'EAT_MoE':
            out, load_balance_loss = self.model(data)
            loss_pred = out[mask]
            loss_true = data.y[mask].view(-1)
            loss = self.loss_metric_kit['loss_fn'](loss_pred, loss_true) + 1e+3 * load_balance_loss
        else:
            out = self.model(data)
            loss_pred = out[mask]
            loss_true = data.y[mask].view(-1)
