...
class PygNodePropPredDataset(InMemoryDataset):
    def __init__(self, name, root='dataset', transform=None, pre_transform=None, meta_dict=None):
        ...
        if meta_dict is None:
            ...
            self.root = osp.join("data", "origin", self.dir_name)
            self.preprocessed_root = osp.join(root, self.dir_name)

            master = pd.read_csv(os.path.join(os.path.dirname(ogb.__file__), 'nodeproppred', 'master.csv'), index_col=0)
    ...
    def processed_dir(self) -> str:
        return osp.join(self.preprocessed_root, 'processed')
    ...
    def process(self):
        ...
        if np.isnan(self.meta_info['additional edge files']):
